...
用一个小型内部运维工具跑通第一条链路。重点是理解产物与门禁，不是第一轮就达到生产就绪。
...
- **门禁**：产品意图获批、Parity 通过、关键约束已确认、浏览器流程真实跑通、人接受结果；
- **下一步**：涉及生产基础设施、Secret、远程执行、多租户、审批或长任务时，升级到 [Full Track](../workflows/full.md)；
- 阶段细节与常见错误：见 [深潜系列](02-prototype-first.md)。

## 相关文档

- [Lightweight Track](../workflows/lightweight.md)：本路径的完整定义与升级条件
- [Full Track](../workflows/full.md)：生产与敏感操作路径
- [Artifact Map](../references/artifacts.md) 与 [Skill Map](../references/skills.md)：产物与 Skill 查表
